...
class astGenPass(tlangVisitor):
    ...
    def visitStart(self, ctx:tlangParser.StartContext):
        self.functions = {}
        self.structs = {}
        for sctx in ctx.structDecl():
            self.visit(sctx)
        for fctx in ctx.functionDecl():
            self.visit(fctx)
    ...
    def visitFunctionDecl(self, ctx): 
        ...
        # self.functions[func_name] = (params, body_ir)
        body_ir = self.visit(ctx.strict_ilist())
        self.functions[func_name] = (params, body_ir)
        return []
    
    def visitStructDecl(self, ctx):
        struct_name = ctx.NAME().getText()
        fields = [v.getText() for v in ctx.VAR()]
        if struct_name in self.structs:
            raise Exception(f"Struct '{struct_name}' already defined.")
        self.structs[struct_name] = fields
        return [(ChironAST.StructDeclaration(struct_name, fields), 1)]
    ...
    # If it's a list of instructions (normal case), extend the list
            if isinstance(visvalue, list):
                instrList.extend(visvalue)

    # If it's a single instruction (like a function call, etc), append it
            else:
                instrList.append(visvalue)
    ...
    def visitAssignment(self, ctx:tlangParser.AssignmentContext):
        lctx = ctx.getChild(0)
        if isinstance(lctx, tlangParser.FieldAccessContext):
            lval = self.visitFieldAccess(lctx)
        else:
    ...
    def visitFunctionCall(self, ctx):
        func_name = ctx.NAME().getText()
        args = [self.visit(arg) for arg in ctx.expression()]
        return ChironAST.CallCommand(func_name, args)  # Treat this as an expression
    ...
    #     struct_expr = ChironAST.StructInit(struct_name)
    #     return [(ChironAST.NoOpCommand(), 1)]   # Placeholder if used as a statement
    def visitStructInit(self, ctx):
        struct_name = ctx.NAME().getText()
        if struct_name not in self.structs:
            raise Exception(f"Struct '{struct_name}' is not defined.")
        return ChironAST.StructInit(struct_name)
    ...
    def visitFieldAccess(self, ctx):
        ...
        field = ctx.NAME().getText()
        return ChironAST.StructFieldAccess(var, field)
    ...
    #     elif ctx.structInit():
    #         return self.visitStructInit(ctx.structInit())
    #     elif ctx.fieldAccess():
    #         return self.visitFieldAccess(ctx.fieldAccess())
    ...
    def visitValue(self, ctx: tlangParser.ValueContext):
        if ctx.NUM():
            ...
            var = ChironAST.Var(ctx.fieldAccess().VAR().getText())
            field = ctx.fieldAccess().NAME().getText()
            return ChironAST.StructFieldAccess(var, field)
